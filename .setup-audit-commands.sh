...
##############################################################################
# AuditEngine Terminal Setup Script
#
# This script embeds audit-specific commands into your terminal environment
# Supports: bash, zsh, and other POSIX shells
#
# Usage: bash .setup-audit-commands.sh
# After running, add this to your ~/.bashrc or ~/.zshrc:
...
ALIASES_FILE="${SCRIPT_DIR}/.audit-aliases.sh"

echo "🚀 Setting up AuditEngine terminal commands..."
echo ""

# Create .audit-aliases.sh
cat > "${ALIASES_FILE}" << 'ALIASES_CONTENT'
#!/bin/bash
...
echo "✅ Created aliases file: ${ALIASES_FILE}"
echo ""
echo "📝 Next steps:"
echo ""
...
# Ask if user wants to add to bashrc/zshrc
read -p "Would you like to auto-add to ~/.bashrc? (y/n) " -n 1 -r
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
  if [ -f ~/.bashrc ]; then
    if ! grep -q "audit-aliases.sh" ~/.bashrc; then
...
read -p "Would you like to auto-add to ~/.zshrc? (y/n) " -n 1 -r
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
  if [ -f ~/.zshrc ]; then
    if ! grep -q "audit-aliases.sh" ~/.zshrc; then
...
echo ""
echo "🚀 Setup complete! You can now use audit commands like:"
echo "   audit-help"
echo "   audit-new"
