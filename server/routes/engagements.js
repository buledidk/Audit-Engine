...
router.post('/', auth.requireAuth, audit.logAction('CREATE_ENGAGEMENT'), async (req, res) => {
  try {
    const {
      entity_id,
      engagement_type,
    ...
    const entity = await db.query('SELECT * FROM entities WHERE id = $1', [entity_id]);
    ...
    const materiality = jurisdictionEngine.calculateJurisdictionMateriality(entityData.jurisdiction_code, {
    ...
    const procedures = jurisdictionEngine.getApplicableProcedures(entityData.jurisdiction_code, 'Revenue', 'Medium');
...
router.get('/:id', auth.requireAuth, audit.logAction('VIEW_ENGAGEMENT'), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const jurisdictionInfo = jurisdictionEngine.getJurisdiction(engagementData.jurisdiction_code);
    ...
    res.json({
      ...
      engagement: {
        ...
        procedures: {
          ...
          percentage: Math.round((engagementData.completed_procedures / engagementData.total_procedures) * 100) || 0,
        },
        findings: {
...
router.get('/', auth.requireAuth, audit.logAction('LIST_ENGAGEMENTS'), async (req, res) => {
  try {
...
router.patch('/:id/status', auth.requireAuth, audit.logAction('UPDATE_ENGAGEMENT_STATUS'), async (req, res) => {
  try {
    const { id } = req.params;
...
router.get('/:id/materiality', auth.requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const materiality = jurisdictionEngine.calculateJurisdictionMateriality(engData.jurisdiction_code, {
...
router.get('/:id/procedures', auth.requireAuth, async (req, res) => {
  try {
    const { id } = req.params;
